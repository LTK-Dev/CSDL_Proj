...
if (empty($_SESSION["user_id"])) {
    ...
    foreach ($_SESSION["cart_item"] as $item) {
        ...
        if ($_POST['submit']) {
            if (isset($_POST["mod"]) && $_POST["mod"] === "COD") {
                $value = "Cash On Delivery";
            } elseif (isset($_POST["mod"]) && $_POST["mod"] === "paypal") {
                $value = "PayPal";
            }
            
            if (!$check) {
                $SQL = "INSERT INTO users_orders (u_id, payment_method) VALUES ('" . $_SESSION["user_id"] . "', '" . $value . "')";
                 mysqli_query($db, $SQL);
                 $check = true;
            }
            ...
            if ($order_id === 0) {
                // Lần đầu tiên trong vòng lặp, gán order_id
                $order_id = $o_id;
            }

            $SQL_2 = "INSERT INTO orders_details (Order_id, Dish_id, Quantity) VALUES ('" . $order_id . "', " . intval($item["d_id"]) . ", " . intval($item["quantity"]) . ")";
            mysqli_query($db, $SQL_2);

            // Xóa sản phẩm từ giỏ hàng
            unset($_SESSION["cart_item"][$item["d_id"]]);
        }
    }

    if ($order_id !== 0) {
        // Đã có ít nhất một sản phẩm được thêm vào đơn hàng
